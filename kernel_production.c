...
// Boot state tracking
static boot_state_t current_boot_state = BOOT_STATE_KERNEL_LOAD;
...
void boot_sequence(void) {
    ...
    splash_init();
    splash_set_theme(SPLASH_THEME_RAEENOS);
    splash_show();
    ...
    for (int i = 0; i < 6; i++) {
        splash_update_progress(i * 15, boot_stages[i]);
        
        switch (i) {
            case 0: // Kernel subsystems
                current_boot_state = BOOT_STATE_KERNEL_LOAD;
                memory_init();
                break;
                
            case 1: // Device drivers  
                current_boot_state = BOOT_STATE_DRIVERS;
                keyboard_init();
                break;
                
            case 2: // Filesystems
                current_boot_state = BOOT_STATE_FILESYSTEM;
                ramfs_init();
                process_init();
                break;
                
            case 3: // Network services
                current_boot_state = BOOT_STATE_NETWORK;
                // Network initialization (placeholder)
                break;
                
            case 4: // Graphics pipeline  
                current_boot_state = BOOT_STATE_GRAPHICS;
                // GPU initialization (placeholder)
                break;
                
            case 5: // User environment
                current_boot_state = BOOT_STATE_USERSPACE;
...
void run_oobe(void) {
    ...
    splash_hide(); // Hide splash for OOBE
    vga_clear();
