...
int snprintf(char* str, size_t size, const char* format, ...) {
    // Very simple implementation for cache naming
    ...
    while (*fmt && remaining > 0) {
        if (*fmt == '%' && *(fmt + 1) == 'z' && *(fmt + 2) == 'u') {
            // Handle %zu format for size_t
            fmt += 3;
            // This is a placeholder - in real implementation would handle va_args
            *out++ = '?';
            remaining--;
        } else if (*fmt != '%') {
            *out++ = *fmt++;
