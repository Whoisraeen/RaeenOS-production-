        ...
        for (uint8_t device = 0; device < 32 && !gpu_found; device++) {
            for (uint8_t function = 0; function < 8; function++) {
                ...
                if (class_code == 0x03) {
                    ...
                    char msg[200];
                    sprintf(msg, "  Vendor: %s (0x%04X)", _gpu_get_vendor_name(vendor_id), vendor_id);
                    debug_print(msg);
                    sprintf(msg, "  Device ID: 0x%04X", device_id);
                    debug_print(msg);
                    sprintf(msg, "  Bus: %d, Device: %d, Function: %d", bus, device, function);
                    debug_print(msg);
                    sprintf(msg, "  Subclass: 0x%02X", subclass);
                    debug_print(msg);
...
static int _gpu_initialize_framebuffer(uint32_t width, uint32_t height, uint32_t depth) {
    ...
    char msg[100];
    sprintf(msg, "Framebuffer initialized: %dx%dx%d (%d KB)", 
            width, height, depth, gpu_device.framebuffer_size / 1024);
    debug_print(msg);
...
int gpu_set_mode(uint32_t width, uint32_t height, uint32_t depth) {
    ...
    char msg[100];
    sprintf(msg, "Setting video mode: %dx%dx%d", width, height, depth);
    debug_print(msg);
